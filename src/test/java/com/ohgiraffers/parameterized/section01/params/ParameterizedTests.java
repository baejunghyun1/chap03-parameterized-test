...
public class ParameterizedTests {
    ...
    * 파라미터로 전달할 값의 몫 만큼 반복적으로 테스트 메소드를 실행하며, 반복 실행 시 마다 준비된 값 목록을
    ...
    * null값과 빈 값을 모두 전달하기 위해 구성된 @NullAndEmptySource 도 이용할 수 있다.
     */
    ...
    void testHasThirtyOneDaysLong(Month month) {
        ...
        //when
        int actual = DateValidator.getLastDatOf(month);
    ...
    * 이 경우 여러 매개변ㅅ에 값을 전달할 여러 인자 묶음이 필요하게 된다.
...
//            delimiter = ':')
    ...
    @DisplayName("CSV 파일을 읽은 테스트 데이터를 테스틍에 활용하는지 확인")
    @ParameterizedTest
    @CsvFileSource(resources = "/parameter-test-data.csv", numLinesToSkip = 1)
    //numLinesToSkip = 1     ->    첫번째줄 출력을 안한다  ()
